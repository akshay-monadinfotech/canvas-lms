...
namespace :sfu do
  ...
  namespace :docker do
    ...
    desc "Generate an access token for the admin user"
    task :generate_admin_access_token => :environment do
      user = User.find(1)
      token = user.access_tokens.create!(:purpose => "Generated by rake task")
      puts "Created access token for #{user.name}: #{token.full_token}"
    end
